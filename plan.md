...
- **Cookie-backed voter memory** — when a user enters a name and submits a vote, write a cookie holding the name and the `ACTIVE_VOTE.number` they voted on. On return visits: prefill the name input; if the cookie matches the open vote, skip the vote UI and land directly on the results view. Invalidate when `ACTIVE_VOTE.number` changes or `ACTIVE_VOTE` is null. Pure client-side, no backend change. Open questions: cookie expiry policy, override path for "voto novamente" (probably no, given the sheet's duplicate-name check), interaction with manual sheet resets between votes.

- **"Estamos a votar" fixture on Votação** — mirror the Início "Estamos a ler" fixture in the Votação area. A compact strip at the top of the Votação view summarising the active vote (vote number, candidate emojis/titles, days open, maybe vote count). Always visible regardless of which sub-state the page is in (name entry, voting, results, thank-you). Goal: keep the vote always visually present in the same way `renderCurrentBook()` keeps the current reading book present on Início. Hides when `ACTIVE_VOTE == null` (same pattern as the Estamos-a-ler block hiding when ARCHIVE is empty). Open questions: what to show — emoji row vs. compact card; whether to suppress it on the results view (where the same info is already on screen).
